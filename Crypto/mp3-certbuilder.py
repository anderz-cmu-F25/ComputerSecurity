...
def make_cert(netid, pubkey, ca_key = ECE422_CA_KEY):
    ...
    builder = builder.subject_name(x509.Name([
        ...
        x509.NameAttribute(NameOID.PSEUDONYM, u'unused' + u'?'*(9+16*3)),
...
if __name__ == '__main__':
    ...
    with open("sol_3.2.4_factorsA.hex") as f:
        lines = f.readlines()
        p = int(lines[0].strip(), 16)
        q = int(lines[1].strip(), 16)

    #with open("sol_3.2.4_factorsB.hex") as f:
    #    lines = f.readlines()
    #    p = int(lines[0].strip(), 16)
    #    q = int(lines[1].strip(), 16)
    ...
    # Creat raw prefix
    # with open("raw_prefix.hex", "wb") as f:
    #     print("raw prefix file created")
